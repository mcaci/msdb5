...
func TestPlayWithSideFindsPlayerInTurn(t *testing.T) {
	...
	if testObject := NewPlayWithSide("", "127.0.0.4", testPlayer, nil, nil, card.Coin); !testObject.Find(testPlayer) {
		t.Fatalf("Unexpected player")
	}
}

func TestPlayWithSideDoesNotFindPlayerNotInTurn(t *testing.T) {
	if testObject := NewPlayWithSide("", "", nil, nil, nil, card.Coin); testObject.Find(player.New()) {
		t.Fatalf("Unexpected player")
	}
}

func TestPlayWithSideNextPlayerOf2is3WithRoundNotEnded(t *testing.T) {
	testObject := NewPlayWithSide("", "", nil, nil, board.New(), card.Coin)
...
func TestPlayWithSideNextPlayerOfAnyIs3WithRoundEnded(t *testing.T) {
	...
	testObject := NewPlayWithSide("", "", nil, nil, testBoard, card.Coin)
...
func TestPlayWithSideNextPhaseWithPlayersWithNonFoldedNameIsTrue(t *testing.T) {
	if testObject := NewPlayWithSide("", "", nil, nil, nil, card.Coin); !testObject.NextPhasePlayerInfo(player.New()) {
...
func TestPlayWithSideNextPhaseWithFoldedPlayerIsFalse(t *testing.T) {
	testPlayer := player.New()
	testPlayer.Hand().Add(1)
	if testObject := NewPlayWithSide("", "", nil, nil, nil, card.Coin); testObject.NextPhasePlayerInfo(testPlayer) {

...
func TestTeam1(t *testing.T) {
	...
	if score1, _ := Calc(fakePlayer, nil, []Piler{fakePlayer}, func(card.Item) uint8 { return 1 }); score1 != 1 {
		t.Fatal("Points string should contain the total of 11")
	}
}

func TestTeam2(t *testing.T) {
	if _, score2 := Calc(nil, nil, []Piler{new(mockPiler)}, func(card.Item) uint8 { return 1 }); score2 != 1 {
		t.Fatal("Points string should contain the total of 11")
	}
}

...
func Play(w http.ResponseWriter, r *http.Request) {
	...
	if info.NextRnd {
		briscola.Collect(info.OnBoard, (*s.Game.Players())[info.NextPl])
		log.Print(s.Game)
		s.Wg.Add(2)
	}
	assets.MustExecute(assets.Label("Player"), w, &struct{ Label interface{} }{Label: pl})
	assets.MustExecute(assets.Label("Next Player"), w, &struct{ Label interface{} }{Label: (*s.Game.Players())[s.Curr].Name()})
}

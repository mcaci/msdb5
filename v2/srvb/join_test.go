...
func TestJoin(t *testing.T) {
	td := []struct {
		...
		{"Join with no data", http.MethodPost, nil, false, testJoinKO, ""},
		// curl -XPOST  -H "Content-Type: application/json" localhost:8080/create -d '{"name":"newgame"}'
		{"Join with game and player name", http.MethodPost, strings.NewReader(fmt.Sprintf(`{"name":"%s","game":"%s"}`, "mary", "test")), true, testJoinOK, "1"},
	...
	for _, tc := range td {
		t.Run(tc.name, func(t *testing.T) {
			const jUrl = "localhost:8080/join"
			res, err := sendJoin(http.NewRequest(tc.method, jUrl, tc.reqBody))
			if err != nil {
...
func sendJoin(req *http.Request, err error) (*http.Response, error) {
	if err != nil {
...
func testJoinOK(res *http.Response, expected string) error {
...
func testJoinKO(res *http.Response, expected string) error {
	if res.StatusCode != http.StatusBadRequest {
		return fmt.Errorf("expected status BadRequest; got %v", res.StatusCode)
	}
